...
#let patterns = (
  element: regex(
    ...
    "(?:" +
      ...
      "((?:_?\d+)|(?:_\([^()]*\)))" +
      "|" +
      // charge: optional ^, optional ., then (roman|digits|single sign|sign+digit)
      // keep support for: ^.2+, ^III-, ^+, +2, .-, etc.
      "(\^?\.?(?:[+-]?(?:\d+|[IV]+)[+-]?|[+-.]))" +
    ")?" +
    // suffix #2: count-or-charge (optional)
    "(?:" +
      "(_?\d+)" +
      "|" +
      // charge2 additionally supports custom ^( ... ) as before
      "(\^\([^)]*\)|\^?\.?(?:[+-]?(?:\d+|[IV]+)[+-]?|[+-.]))" +
    ")?" +
    // oxidation: ^^(+/-)?(1-3 roman I/V or digits)
    "(\^\^[+-]?(?:[IViv]{1,3}|\d+))?",
  ),
